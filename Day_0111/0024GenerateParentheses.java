...
class GenerateParentheses{
    ...
    private void backtrack(StringBuilder partialSolution, List<String> solutions, int openingBraces, int closingBraces, int n) {
        ...
        for (String brace : candidates) {
            ...
            backtrack(partialSolution,
                    solutions,
                    brace.equals("(") ? openingBraces + 1 : openingBraces,
                    brace.equals(")") ? closingBraces + 1 : closingBraces,
                    n);
    ...
    private List<String> generateCandidates(int openingBraces, int closingBraces, int n) {
        ...
        if (openingBraces < n) { // we always have room for appending one more"("
            // at the to the partialSolution
            // anytime as long as partialSolution contains less than n opening braces.
            // Keep in mind we cannot have more than n opening braces in our solution.
            candidates.add("(");
        }
        if (closingBraces < openingBraces) { // we can append ")" to our partial solution
